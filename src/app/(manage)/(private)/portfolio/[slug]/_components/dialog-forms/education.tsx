...
const EducationFormDialog = ({
  ...
  const { mutateAsync: upsertEducation, isPending } =
    api.portfolio.upsertEducation.useMutation({
      onSuccess: async () => {
      ...
      <DialogContent className="min-w-[93%] sm:min-w-[90%] lg:min-w-3xl">
        <DialogHeader>
          <div className="flex items-center gap-2">
            ...
            <DialogTitle className="text-base font-medium md:text-base lg:text-base">
              {open === "create" ? "Add Education" : "Update Education"}
            </DialogTitle>
          </div>
        </DialogHeader>
        ...
        <Form {...educationForm}>
          <form onSubmit={educationForm.handleSubmit(onSubmit)}>
            <div className="grid max-h-[65vh] grid-cols-1 gap-3 overflow-y-auto px-1 sm:max-h-[80vh] md:grid-cols-2">
              <FormField
